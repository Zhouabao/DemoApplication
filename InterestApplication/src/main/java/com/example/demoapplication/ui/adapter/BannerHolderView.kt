 ...
 *    version: 1.0
 */
class BannerHolderView(itemView: View,var context:Context) : Holder<BannerBean>(itemView) {
    override fun updateUI(data: BannerBean) {
        itemView.banner_name.text = data.title
        itemView.banner_content.text = data.content
        GlideUtil.loadImg(context, data.img, itemView.banner_img)
    }
