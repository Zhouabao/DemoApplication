package com.example.demoapplication.widgets.stackview;
...
import com.example.demoapplication.widgets.stackview.rebound.*;
...
public class SwipeTouchHelper implements ISwipeTouchHelper {
    ...
    private void doManualDisappear(final int direction) {
        ...
        } else if (direction == StackCardsView.SWIPE_UP) {
...
//            }
    ...
    private boolean doFastDisappear(float vx, float vy) {
        ...
        final float dx = disappearView.getX() - initX;
        final float dy = disappearView.getY() - initY;
        ...
        animator.addListener(new AnimatorListenerAdapter() {
            ...
            public void onAnimationEnd(Animator animation) {
                mDisappearingCnt--;
                if (dy < 0)
                    mSwipeView.onCardDismissed(StackCardsView.SWIPE_UP); //// // FIXME
                else if (dx > 0)
                    mSwipeView.onCardDismissed(StackCardsView.SWIPE_RIGHT); //// // FIXME
                else
                    mSwipeView.onCardDismissed(StackCardsView.SWIPE_LEFT); //// // FIXME
    ...
    private ScrollInfo calcScrollInfo(View movingView) {
        ...
        if (Float.compare(dx, 0) == 0 && Float.compare(dy, 0) == 0) {
            ...
            if (Math.abs(dx) * SLOPE > Math.abs(dy)) {
                ...
                direction = StackCardsView.SWIPE_UP;
            }
        }
