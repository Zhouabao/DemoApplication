...
public class WaterRippleView extends View {
    ...
    private GradientDrawable mBitmap;
    private int mWidth;
    private int mHeight;
    ...
    private void initAttrs(Context context, AttributeSet attrs) {
        ...
        typedArray.recycle();
        mBitmap = (GradientDrawable) drawable;
        mPaint = new Paint();
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        int size = (mRippleCount * mRippleSpacing + mBitmap.getIntrinsicWidth() / 2) * 2;
        mWidth = resolveSize(size, widthMeasureSpec);
        mHeight = resolveSize(size, heightMeasureSpec);
        setMeasuredDimension(mWidth, mHeight);
        mMaxStrokeWidth = (mWidth - mBitmap.getIntrinsicWidth()) / 2;
        initArray();
    }
...
//        drawBitmap(canvas);
    ...
    private void drawBitmap(Canvas canvas) {
        int left = (mWidth - mBitmap.getIntrinsicWidth()) / 2;
        int top = (mHeight - mBitmap.getIntrinsicWidth()) / 2;
//        canvas.drawBitmap(mBitmap, left, top, null);
    ...
    private void drawRipple(Canvas canvas) {
        for (int strokeWidth : mStrokeWidthArr) {
            ...
            canvas.drawCircle(mWidth / 2, mHeight / 2, mBitmap.getIntrinsicWidth() / 2 + strokeWidth / 2,
                    mPaint);
        }
        for (int i = 0; i < mStrokeWidthArr.length; i++) {
            if ((mStrokeWidthArr[i] += 2) > mMaxStrokeWidth) {
                mStrokeWidthArr[i] = 0;
            }
        }
    }
