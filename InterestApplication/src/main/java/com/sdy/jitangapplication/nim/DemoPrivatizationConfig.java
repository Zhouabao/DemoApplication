...
public class DemoPrivatizationConfig {
    ...
    private static ServerAddresses parseAddresses(JSONObject jsonObject) {
        ...
        addresses.handshakeType =
                NimHandshakeType.value(jsonObject.optInt(KEY_HAND_SHAKE_TYPE, NimHandshakeType.V1.getValue()));
    ...
    private static boolean checkFormatValid(String format) {
        return !TextUtils.isEmpty(format) && format.contains(BUCKET_NAME_PLACE_HOLDER)
